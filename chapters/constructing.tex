...
\chaptermark{Constructing QIIDs}
\label{constructing}
...
inductive-inductive definitions. They key is here to not only
construct an initial object but establish that there is a left adjoint
to the forgetful functor $\Func{\Alg_{n+1}}{\Alg_{n}}$, which allows
us to do the construction by induction on the length of the
specification. We end up with a chain of adjunctions:
$$
\xymatrix{
\SET \ar@/^/[r]^{L_0}
\ar@{}[r]|{\top} 
...
\begin{theorem}
  ...
  inductive-inductive definition. If all the arguments functors in
