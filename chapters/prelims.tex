...
When considering type familes $B : A \to \Type$ over some type $A$, we
...
\begin{definition}
  ...
  \begin{align*}
    &idtoequiv : (A = B) \to \Iso{A}{B} \\
    &idtoequiv \ddefeq \J\ (\lambda A\ B\ p . \Iso{A}{B}))\ (\lambda A \to id-iso\ A)\ 
  \end{align*}
\end{definition}
...
eachother, so we do not have such a statement for $\Sigma$-types. In
...
$$
x =_{A}^{'} y \ddefeq (z : A) \to z = x \to z = y
$$
...
We can think of a function $\lambda z . z = x$ as a representible
functor. A function of type $(z : A) \to z = x \to z = y$ is
necessarily a natural transformation. Under this association, we can
formulate the Yoneda lemma for identity types:
...
The equivalence we get from the Yoneda lemma for identity types. This
equivalence also maps reflexivitiy and transitivity of the original
identity types onto composition of the alternative version and vice
versa. The alternative identity types also satisfy the path induction
principle.
