...
In this section we will work out the description of the
$(\ttconty, \tttyty)$ type family of contexts and types in a context
as described in \cref{syntax-of-type-theory}. First of all, notice
that it is a $\FAM$-sorted definition. In \cref{fam-sorts}, we have
seen that these can be described with the following functors:
%
\begin{itemize}
\item $R_{\ttconty} : \Func{\termcat}{\SET}$, $R_{\ttconty}\ x \ddefeq \unitty$
\item $R_{\tttyty} : \Func{\SortCat{R_{\ttconty}}}{\SET}$, $R_{\tttyty} (x, A) \ddefeq A\ x$
\end{itemize}
%
This gives us the sorts category $\SortCat{R_0,R_1}$ with:
\begin{itemize}
\item objects:
  $(x : \unitty) \times (A : \unitty \to \Set) \times (B : A\ x \to
  \Set)$
\item morphisms $(x, A, B) \to (y, C, D)$:
  $(f : \unitty \to \unitty) \times (g : (x : \unitty) \to A\ x \to C\
  (f\ x)) \times (h : (a : A\ x) \to B\ a \to D\ (g\ x\ a))$
\end{itemize}
To not clutter things too much with terms of type $\unitty$, we will
leave these out of the sorts part of the definition. The
inductive-inductive definition we are trying to fit into the framework
we just presented is as follows, massaged a bit to see more directly
how it fits in the framework:
%
\begin{sorts}
  \sortnamety{\ttconty}{\unitty \to \Set} \\
  \sortnamety{\tttyty}{\ttconty\ \unitval \to \Set}
\end{sorts}
%
\begin{datatype}{\ttconty}{}
  \constr{\ttnil}{\unitty \to \ttconty\ \unitval} \\
  \constr{\ttcons{\_}{\_}}{(\Gamma : \ttconty\ \unitval) \times (\tau : \tttyty\ \Gamma) \to \ttconty\ \unitval}
\end{datatype}
%
\begin{datatype}{\tttyty}{}
  \constr{\ttzero}{(\Gamma : \ttconty\ \unitval) \to \unitty \to \tttyty\ \Gamma} \\
  \constr{\ttone}{(\Gamma : \ttconty\ \unitval) \to \unitty \to \tttyty\ \Gamma} \\
  \constr{\ttpi}{(\Gamma : \ttconty\ \unitval) \to (A : \tttyty\ \Gamma) \times \tttyty\ (\ttcons{\Gamma}{A}) \to \tttyty\ \Gamma}
\end{datatype}

The specification $s_{(\ttconty,\tttyty)} : \specty$ is a list of five
elements, all containing a point constructor. Of the point
constructors, the first two are of the first sort, $\unitty \to \Set$,
and the last three of sort $\ttconty\ \unitval \to \Set$. The
following functors describe the point constructors:
\begin{itemize}
\item $F_{\ttnil}\ (A, B) \ddefeq (\lambda \_ . \unitty)$
\item $F_{\ttcons{\_}{\_}}\ ((A,B),\theta_{\ttnil}) \ddefeq (\lambda \_ . (\Gamma : A\ \unitval) \times (\tau : B\ \Gamma))$
\item $F_{\ttzero}\ ((A,B),\theta_{\ttnil},\theta_{\ttcons{\_}{\_}}) \ddefeq (A, \lambda \_ . \unitty)$
\item $F_{\ttone}\ ((A,B),\theta_{\ttnil},\theta_{\ttcons{\_}{\_}},\theta_{\ttzero}) \ddefeq (A, \lambda \_ . \unitty)$
\item $F_{\ttpi}\ ((A,B),\theta_{\ttnil},\theta_{\ttcons{\_}{\_}},\theta_{\ttzero},\theta_{\ttone}) \ddefeq (A, \lambda \Gamma . (X : B\ \Gamma) \times B\ (\theta_{\ttcons{\_}{\_}}\ (\Gamma, X)))$
\end{itemize}
The algebras we end up with, after singleton contraction, look as
follows:
%
\begin{align*}
&&&\ (A : \unitty \to \Set) \times (B : A\ \unitval \to \Set) \\
&&\times&\ (\theta_{\ttnil} : \unitty \to A\ \unitval) \\
&&\times&\ (\theta_{\ttcons{\_}{\_}} : (\Gamma : A\ \unitval) \times B\ \Gamma \to A\ \unitval) \\
&&\times&\ (\theta_{\ttzero} : (\Gamma : A\ \unitval) \to \unitty \to B\ \Gamma) \\
&&\times&\ (\theta_{\ttone} : (\Gamma : A\ \unitval) \to \unitty \to B\ \Gamma) \\
&&\times&\ (\theta_{\ttpi} : (\Gamma : A\ \unitval) \to (X : B\ \Gamma) \times B\ (\theta_{\ttcons{\_}{\_}}\ (\Gamma, X)) \to B\ \Gamma)
\end{align*}
%
If we squint our eyes a bit, we see that the algebras indeed coincide
with the constructors we wanted to give a specification of.

The pieces of the specification we have left out here for notational
clarity, are the sort membership proofs. An example of these proofs
has been given in \cref{sortsin-example}, also for the same sorts
specification as the one we use here.
...
\theta : (x : FX) \to G (X , x)
$$
The category $\int_{\Cc} F$ is the \emph{category of elements} or the
\emph{Grothendieck construction} of the functor $F$. It is defined as having:
\begin{itemize}
\item objects: $(X : | \Cc |) \times FX$
\item morphisms $(X,x) \to (Y,y)$: $(f : \Cc(X,Y)) \times (F\ f\ x = y)$
\end{itemize}
In other words: the category of elements has as objects objects in $X$
along with a point in $FX$. Morphisms are morphisms in $\Cc$ that
preserve the points.

One nice feature of this approach is that it gives us a unified
framework to talk about both path constructors as well as point
constructors of various sorts. 

However, formalising the category structure of dependent dialgebras
turns out to be rather involved. In particular, when defining what
morphisms between these algebras should be, we need to lift a morphism
in $\Cc$ to one in $\int_{\Cc} F$.  There is a canonical way of doing
so, given an $x : FX$ for some $X : | \Cc |$: given $f : \Cc(X,Y)$
with $x : FX$, we define:
%
\begin{align*}
...
have $\widehat{g \circ f}_x$ and $\hat{g}_{F f x} \circ \hat{f}_x$ are
equal. In fact, they have different types:
\begin{itemize}
  \item $\widehat{g \circ f}_x : \int_{\Cc} F ((X,x),(Z,F\ (g \circ f)\ x))$
  \item $\hat{g}_{F f x} \circ \hat{f}_x : \int_{\Cc} F ((X,x),(Z, F\ g\ (F\ f\ x)))$
\end{itemize}
While we do have a proof that $F\ (g \circ f)\ x = F\ g\ (F\ f\ x)$,
we do not have in general that this equation holds
definitionally. Writing down the category structure therefore involves
lots of transporting across the proofs of functoriality of $F$ and
reasoning about these transports.

In the approach taken in this thesis, we build up the category of
algebras using easier to define building blocks, \ie equaliser
categories and categories of (ordinary) dialgebras. While this
approach still has its share of having to deal with transporting terms
across proofs of functoriality, these can be solved in a more abstract
setting with fewer moving parts.

Another reason why this thesis does not use dependent dialgebras is
that they are too general for our purposes. If we try to prove
properties about them, \eg the existence of limits and initial
objects, it is not immediately clear what restrictions to put on the
target functor $G$ to make things work. In this thesis we use a more
bottom up approach, starting from the inductive definitions we know
and what functors show up when defining them.
