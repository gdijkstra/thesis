...
It has been shown\todo{Backhouse reference} that the type formation
and introduction rules are enough to derive the rest from. This is
also reflected in how one gives an inductive definition in proof
assistants such as Agda and Coq: we give the type signature of the
...
\todoi{Mention that this gives us a left adjoint to the forgetful
  functor, hence $F^*$ is a monad}
...
\end{theorem}
\todo{Refer to polynomial monad paper by Kock/Gambino}
...
\todoi{Mention that this is the approach taken in the
  Shulman/Lumsdaine paper. While our approach is definitely similar,
  we consider the forgetful functors and left adjoints
  \emph{separately}: we decouple the definition of algebras and such
