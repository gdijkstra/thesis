...
\item \emph{computation rules}: these specify how the elimination
  rules interact with the introduction rules
\end{itemize}

It has been shown that the type formation and introduction rules are
enough to derive the rest from\cite{Backhouse1989}. This is also
reflected in how one gives an inductive definition in proof assistants
such as Agda and Coq: we give the type signature of the inductive
definition (its type formation rules) and a list of constructors (its
introduction rules).

There are several equivalent ways to describe inductive definitions:
we can formulate a syntax of constructors and describe the elimination
principle by induction on terms in this syntax. This syntax is a
subset of the syntax of type theory: we need to be able to write down
a list of constructors which is just a list of types of a certain
form. One can take an ``analytic'' approach to this: take the syntax
of type theory and define a predicate on it that tells us whether a
term is in the appropriate subset. This is effectively what systems
like Agda do: you can write down a list of constructors and Agda
checks whether it makes sense, \eg whether the result types of the
constructors are correct and whether the arguments are strictly
positive. Doing this in type theory itself will be rather involved, as
formalising the syntax of type theory in type theory is generally not
...
$\Set$, which is something we can concisely formalise in type
theory. The functors can be described as \emph{containers} and the
category of algebras on a container can also be readily
formalised. Stating that it is initial only requires us to have
defined the objects and morphisms of this category. 
...
computation rules. This categorical setting allows us to generalise
without having to keep track of too many moving parts, but on the
other hand also is not so overly abstract that it sacrifices intuition
for conciceness.
...
induction principle generally requires a bit more machinery, as does
showing its logical equivalence to initiality. This is covered in
\cref{induction}.
...
We start out by reviewing initial algebra semantics of ordinary
inductive types in \cref{algebraic-semantics}. In
...
to previous constructors. \Cref{dependent-sorts} formalises the notion
of \emph{dependent sorts}, which deals with the type formation rules
for quotient inductive-inductive definitions. In
...
form of iterated dialgebras with equations. This is followed by a
discussion on whether we can generalise this idea to a larger class of
dialgebras in \cref{other-forms-of-constructors}, allowing for a more
general shape of constructors. \cref{from-syntax-to-functors} how the
formal inductive definitions differ from what one would like to write
down in type theory itself. In
\cref{application-to-equational-theories} we see how the formalisation
of quotient inductive-inductive definitions can also be used to talk
about equational theories.
...
\begin{proof}
  ...
  $$
  using the following equation reasoning:
  \begin{align*}
    &&&X \to Y \\
...
of $\algcat{F}$ whose carrier is $F^* \initobj$, which is the
inductive type as defined by the endofunctor $F$.
...
defined in terms of generators and relations. In \cite{Lumsdaine2013},
the authors give semantics of 1-HITs by constructing these presented
monads. As these constructions are done in a wide class of model
...
the categories of algebras in this chapter. In \cref{constructing} we
show how one can construct initial algebras and left adjoints to the
forgetful functors of categories of algebras. The reason why we chose
to do it this way is that it allows us to talk about algebras (the
``syntax'') separately from their existence (``semantics'').
...
structure on $\Tty$, with $F_0 : \Set \to \Set$ being defined as
...
forgetful functor $V_0 : \Func{\algcat{F_0}}{\Set}$ which gives us the
carrier of the algebra. We therefore have that
$\Tb : F_1(\Tty,\Ta) \to V_0(\Tty,\Ta)$, \ie $\Tb$ is an
$(F_1,V_0)$-\emph{dialgebra}~\cite{Hagino1987} structure on
$(\Tty,\Ta) : \algcat{F_0}$.
%
\begin{definition}
  \label{dialg}
...
functor $V_1 : \dialgcat{F_1}{V_0} \to \algcat{F_0}$. If we were to
add a third and a fourth constructor, we would have to define a
functors $F_2 : \dialgcat{F_1}{V_0} \to \Set$ and
$F_3 : \dialgcat{F_2}{U_1} \to \Set$ to describe their
...
the categories of dialgebras have a forgetful functor into $\Set$. We
can formalise the concept $\Set$-sorted inductive-inductive
...
$R : \Func{\SortCat{s}}{\Set}$, we can define a forgetful functor
...
\[
\xymatrix{
1 &\Set \ar[l]_{t_0} &\Rel \ar[l]_{t_1}
}
\]
\end{example}
...
\begin{datatype}{\sortsin{\_}{\_}}{\Cat \to \sortsty \to \Set}
  \constr{\sortsinhere}{(s : \sortsty)\ (R : \Func{\SortCat{s}}{\Set}) \to \sortsin{\SortCat{\sortssnoc\ s\ R}}{\sortssnoc\ s\ R}} \\
  \constr{\sortsinthere}{(s : \sortsty)\ (R : \Func{\SortCat{s}}{\Set})\ (\Cc : \Cat) \to \sortsin{\Cc}{s} \to \sortsin{\Cc}{\sortssnoc\ s\ R}}
\end{datatype}
%
\begin{example}
...
\item $T : (x y z : O) \to A\ x\ y \to A\ y\ z \to A\ x\ z \to \Set$ (a family of triangles)
\end{itemize}
%
...
constructors. In this section we will generalise that construction to
0-constructors of arbitrary sort as well as introduce 1-constructors.

\subsection{A $\Rel$-sorted quotient inductive-inductive type}

As a warm-u we will give an example of how the specification and the
...
The category of sorts is the category $\Rel$. In \cref{rel-sorts} we
saw how this can be represented as a list of functors. This category
is also the first category of algebras, \ie algebras with no
constructors, and will as such also be referred to as $\Alg_0$.
...
defined by $F_1\ (X, R, \theta_0) \ddefeq \unitty$. $\Alg_2$ has
objects $(X : \Set) \times (\theta_0 : X) \times (\theta_1 : X)$, or
...
$\Rel$. $\ell_{((X,Y),\theta_0,\theta_1,\theta_2)}$ and
$r_{((X,Y),\theta_0,\theta_1,\theta_2)}$ are defined as
$(\id_X, \ell^1_{((X,Y),\theta_0,\theta_1,\theta_2)})$ and
$(\id_X,r^1_{((X,Y),\theta_0,\theta_1,\theta_2)})$ respectively, with
%
\begin{align*}
&\ell^1_{((X,Y),\theta_0,\theta_1,\theta_2)}\ a\ b\ (p, q, n) \ddefeq \theta_2'\ a\ b\ p\ q\ n \\
...
$\Alg_3$ has objects $(X : | \Alg_2 |) \times (\theta_3 : \ell_X = r_X)$,
...
\item
  $\ell^1, r^1 : (X : | \Cc |) (x : R_i (\bar{U}_0\ X)) (y : F_1\ X\ x)
  \to \bar{U}_1\ X\ x$
  such that $\ell_X \ddefeq (\id_{\bar{U}_0 X}, \ell^1)$ and
...
\eqref{eq:theta} is equivalent to $\ell^1_X = r^1_X$. Since
$t_i\ (l\ X) = t_i\ (r\ X) = \id_{t_i \circ
  \bar{U}}$, %: S_{i-1} (t_i(\bar{U} X), t_i (\bar{U} X))$,
the equality $\ell_1\ X = r_1\ X$ of dependent functions in
...
If we look at the 0-constructors we have seen so far, they all are of
the shape:
$$
\Ac : \Alg_s(F X , U X)
$$
...
list of functors and natural transformations, whilst at the same time
defining the categories of algebras that correspond these
definitions. In a proof assistant, we expect to specify an inductive
definition by giving a list of constructors, \ie a list of types that
adhere to certain rules. Our approach is somewhat similar to this: we
essentially have to give a functor for each constructor specifying the
arguments and give natural transformations to indicate the end points
if it happens to be a path constructor we are describing. To give
these data, we may need to massage our types before we can describe
them as functors. 
...
recursion principle, but as we only talk about computational behaviour
up to propositional equality, this does not matter in our setting.
...
\subsection{Comparison to Agda's inductive definition}
...
\todo{Move this to a final concluding chapter?} Now that we have a
mechanism that gives us the category of algebras given a list of
constructors, we can also use this to create the category of models
for an equational theory. For example, if we take the axioms of
setoids as constructors, the corresponding category of algebras is
equivalent to the category of setoids. A setoid consists of the
following data:
\begin{itemize}
\item $X : \Set$
\item $R : X \to X \to \Set$
\item $\mathsf{refl} : (x : X) \to R\ x\ x$
\item $\mathsf{sym} : (x\ y : X) \to R\ x\ y \to R\ y\ x$
\item $\mathsf{trans} : (x\ y\ z : R) \to R\ x\ y \to R\ y\ z \to R\ x\ z$
\item $\mathsf{is-prop} : (x\ y : R) \to (p\ q : R\ x\ y) \to p = q$
\end{itemize}
As an inductive definition, this is not terribly interesting as the
initial algebra for this would consist of the empty type with an
equivalence relation.

\todoi{Explain why what we get is really the category of setoids: for
  this we just need to look at the morphisms in the sort category, \ie
  $\Rel$}
