...
free monad need not exist: $F$ is required to be an strictly positive
...
\begin{proof}
  ...
  $$
  using the following equational reasoning in \cref{f-star-monad}:
...
the same as the class of ordinary inductive types. A generalisation of
ordinary inductive types would then be similar to considering a larger
class of monads than just free monads. In \cite{Shulman2011} the
author proposes that higher inductive type should correspond to a
notion of \emph{presented monad}, \ie a monad that is defined in terms
of generators and relations, or in HIT terminology: point and path
...
\begin{example}
  The natural numbers can be represented both in the old-fashioned way
  with one endofunctor $F X \ddefeq \unitty + X$. In our framework, we can
  treat the constructors separately, staying a bit closer to the
  syntax. We define:
  \begin{align*}
    &F_0 : \Func{\SET}{\SET} \\
    &F_0\ X \ddefeq \unitty \\
  ...
  The category of algebras for the natural numbers definition is then
  $\Alg_{[F_0,F_1]}$.
\end{example}
...
$\extendfunc{\Und} : \Func{\Cc}{S_i}$ for any $S_i$ in the chain by composing
...
functor $\Und : \Func{\Cc}{\SortCat{\Ss}}$, we can define the function:
$$
\extendfunc{\Und} : (S_i : \Cat) \to \sortsin{S_i}{\SortCat{\Ss}} \to \Func{\Cc}{S_i}
$$
This function is defined by induction over the proof of
$\sortsin{S_i}{\SortCat{\Ss}}$. We will usually just write
$\extendfunc{\Und} X$, leaving the membership proof implicit.
