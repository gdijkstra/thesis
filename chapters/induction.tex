...
\begin{proof}
  ...
  $\phi_U : \Alg_s(U(X \times Y), UX \times UY)$ as
  ...
  We furthermore have to check whether $\pi_0 : \Alg_s (X \times Y, X)$
  and $\pi_1 : \Alg_s (X \times Y, Y)$ are algebra morphisms. To this
...
\begin{lemma}[Equalisers of 1-constructors]
  Let $s' : \specty_\Ss$ be $s$ extended with a 1-constructor of sort
  $S_i$ as specified by:
  \begin{itemize}
  \item $F : \Func{\Alg_s}{S_i}$, satisfying $t_i \circ F = t_i \circ \bar{U_s}$
  \item $l, r : \Nat{F}{\bar{U}_s}$ satisfying
