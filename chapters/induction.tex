...
\begin{lemma}[Products of 0-constructors]
...
\begin{proof}
  ...
  By the assumption that $\bar{U_s}$ preserves products, we have an
  isomorphism:
  $$
  \phi : \bar{U_s} X \times \bar{U_s} Y \to \bar{U_s}(X \times Y)
  $$
  Let $\Delta_X : X \to X \times X$ be the diagonal morphism. $\phi$
  is the inverse of
  $ \bar{U_s \pi_0} \times \bar{U_s \pi_1} \circ \Delta$. Using this
  isomorphism we can then go on and define the algebra structure on
  $X \times Y$ as the following composite:
  $$
  ...
  $$
  We need to check whether this 

\end{proof}

\begin{lemma}[Products of 1-constructors]
...
\begin{lemma}[Equalisers of 0-constructors]
...
\begin{lemma}[Equalisers of 1-constructors]
