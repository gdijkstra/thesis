...
%
  \begin{alignat*}{2}
    &\Fam_{\Alg_{\lambda X . 1 + X}} : | \Alg_{\lambda X . 1 + X} | \to \Set \\
...
\begin{proof}
  ...
  \xymatrix{ 
    ...
    &\extendfunc{U}X \times \extendfunc{U}Y \ar[r]^{\phi_U^{-1}} 
    &\extendfunc{U}(X \times Y) 
  }
  $$
  ...
  \xymatrix{
    ...
    &\extendfunc{U}X \times \extendfunc{U}Y \ar[r]^{\phi_U^{-1}} \ar[dr]_{\pi_0}
    &\extendfunc{U}(X \times Y) \ar[d]_{U \pi_0}
    \\
    FX \ar[rrr]_{\theta} &&&UX
  }
  $$
  ...
  Note that by construction, $\extendfunc{U}_{s'}$ preserves products
  definitionally.
\end{proof}
...
\begin{lemma}[Products of path constructors]
  ...
  \item $l, r : \Nat{F}{\extendfunc{U}_s}$ satisfying
    $t_i\ l = t_i\ r = \id_{t_i \circ \extendfunc{U}_s}$.
  \end{itemize}
...
\begin{proof}
  ...
  $\extendfunc{U}_{s'}$ preserves products definitionally.
\end{proof}
...
\begin{proof}
  ...
  $t_i \epsilon = \id_{t_i (\extendfunc{U}_s E)}$. We know that the following
  commutes:
  $$
  \xymatrix{
  ...
  $$
  Furthermore, $t_i \theta = \id_{t_i (\extendfunc{U}_s X)}$ and
  $t_i (Fe) = t_i (Ue)$, and since $t_i$ preserves equalisers, we
  necessarily have that $t_i \epsilon = \id_{t_i (\extendfunc{U}_s E)}$.
...
$$
\liftdephom{F} : (X : | \Cc |) (P : \Fam_{\Cc}\ X) \to \DepHom_{\Cc}\ X\ P \to \DepHom_{\Cc}\ FX\ (\Box_F\ X\ P)
$$
If $F$ happens to be an endofunctor on $\SET$, this simplifies to:
$$
\liftdephom{F} : (X : \Set) (P : X \to \Set) \to ((x : X) \to P\ x) \to (x : F X) \to \Box_F\ X\ P\ x
$$
...
$\dialgcat{F}{U}$ with $U : \Func{\Alg_s}{\SET}$ the forgetful
functor. Following a similar derivation to that of the family
structure on $\algcat{F}$ for endofunctors $F$ on $\SET$, we arrive
at:
$$
\Fam_{\dialgcat{F}{U}}\ (X,\theta) \ddefeq (P : \Fam_{\Alg_s}\ X)
\times (m : (x : FX) \times \Box_F\ P\ x \to \Box_U\ P\ (\theta\ x))
$$
We can recover $\Fam_{\algcat{F}}$ from this if $\Alg_s = \SET$ and
$U$ is the identity functor, as $\Box_{\id_{\SET}}\ P = P$.
...
\item $F : \Func{\Alg_s}{S_i}$ such that $t_i \circ F = t_i \circ \extendfunc{U}_s$
\end{itemize}

We start out by investigating what $\Box_F$ and $\Box_{\extendfunc{U}_s}$ look
like. Recall that we can split the functor $F : \Func{\Alg_s}{S_i}$
into two operations, which are both functorial:
\begin{itemize}
\item $F^0 : | \Cc | \to |S_{i-1}|$, which we know to be equal to $\extendfunc{U}^0_s$
\item
...
$F^0 = \extendfunc{U}^0_s$, we will use $\Box_{R_i \circ \extendfunc{U}^0_s}$.
...
%
\begin{align*}
\Fam_{\Alg_{s'}}\ (X, \theta) \ddefeq &(m : (x : R_i (\extendfunc{U}^0_s X)) \times (x' : \Box_{R_i \circ \extendfunc{U}^0_s}\ P\ x) \\
& \to (y : F^1\ X\ x) \times (y' : \Box_{F^1}\ P\ (x, x')\ y) \\
& \to \Box_{\extendfunc{U}^1_s}\ P\ (x,x')\ (\theta\ x\ y))
\end{align*}
%
\begin{figure}\centering
  \begin{align*}
    &&&(Y : | \Alg_s |) \\
    &&&\times (\rho : S_i(FY, \extendfunc{U}_s Y)) \times (t_i \rho = \id_{t_i (\extendfunc{U}_s X)}) \\
    &&&\times (\Alg_{s'} ((Y,\rho),(X,\theta)) \\
    &=&&\reasontext{unfold definition of $\Alg_{s'}$} \\
    &&&(Y : | \Alg_s |) \times (\rho : S_i(FY, \extendfunc{U}_s Y)) \times (t_i \rho = \id_{t_i (\extendfunc{U}_s Y)}) \\
    &&&\times (p : \Alg_s(Y,X)) \times (p_0 : \extendfunc{U}_s p \circ \rho = \theta \circ F p) \\
    &=&&\reasontext{combine $Y$ and $p$ using $\Fam$ structure on $\Alg_s$} \\
    &&&(P : \Fam_{\Alg_s}\ X) \times (\rho : S_i(F(\total\ P), \extendfunc{U}_s (\total\ P))) \times (t_i \rho = \id_{t_i (\extendfunc{U}_s (\total\ P))}) \\
    &&&\times (p_0 : \extendfunc{U}_s(\proj\ P) \circ \rho = \theta \circ F (\proj\ P)) \\
    &=&&\reasontext{unfold definition $S_i$ and expand $p_0$} \\
    &&&(P : \Fam_{\Alg_s}\ X) \\
    &&&\times (\rho : (x : R_i (\extendfunc{U}^0_s (\total\ P))) \to F^1 (\total\ P)\ x \to \extendfunc{U}^1_s (\total\ P)\ x) \\
    &&&\times (p_0 : (x : R_i (\extendfunc{U}^0_s (\total\ P)))\ (y : F^1 (\total\ P)\ x) \\
    &&&\ \ \ \ \to \extendfunc{U}^1_s (\proj\ P)\ x (\rho\ x\ y) = \theta\ (R_i\ (\extendfunc{U}^0_s (\proj\ P) x)\ (F^1\ (\proj\ P)\ x\ y))) \\
    &=&&\reasontext{rewriting using $\Box$ notation} \\
    &&&(P : \Fam_{\Alg_s}\ X) \\
    &&&\times (\rho : (x : R_i (\extendfunc{U}^0_s X)) \times (x' : \Box_{R_i \circ \extendfunc{U}^0_s}\ P\ x) \to (y : F^1\ X\ x) \times (y' : \Box_{F^1}\ P\ (x, x')\ y) \\
    &&&\ \ \ \ \to (z : \extendfunc{U}^1_s\ X\ x) \times \Box_{\extendfunc{U}^1_s}\ P\ (x,x')\ z) \\
    &&&\times(p_0 : (x : R_i (\extendfunc{U}^0_s X)) \times (x' : \Box_{R_i \circ \extendfunc{U}^0_s}\ P\ x) \to (y : F^1\ X\ x) \times (y' : \Box_{F^1}\ P\ (x, x')\ y) \\
    &&&\ \ \ \ \to \pi_0\ (\rho\ (x,x')\ (y,y')) = \theta\ x\ y) \\
    &=&&\reasontext{singleton contraction} \\
    &&&(P : \Fam_{\Alg_s}\ X) \\
    &&&\times (m : (x : R_i (\extendfunc{U}^0_s X)) \times (x' : \Box_{R_i \circ \extendfunc{U}^0_s}\ P\ x) \to (y : F^1\ X\ x) \times (y' : \Box_{F^1}\ P\ (x, x')\ y) \\
    &&&\ \ \ \ \to \Box_{\extendfunc{U}^1_s}\ P\ (x,x')\ (\theta\ x\ y))
  \end{align*}
...
$(x : R_i (\extendfunc{U}^0_s X)) \times (x' : \Box_{R_i \circ \extendfunc{U}^0_s}\
P\ x)$,
...
$\Box_{\extendfunc{U}^1_s}\ P\ (x,x')\ (\theta\ x\ y)$, which is the
...
$(X,\theta) : | \dialgcat{F}{U} |$ and
$(P,m) : \Fam_{\dialgcat{F}{U}}\ (X,\theta)$, we have:
$$
\DepHom_{\dialgcat{F}{U}}\ (P, m) \ddefeq (\sss : \DepHom_{\Alg_s}\ P) \times (\sss_0 : (x : FX) \to \liftdephom{U}\ \sss\ (\theta\ x) = m\ x\ (\liftdephom{F}\ \sss\ x))
$$
...
\begin{align*}
  ...
  &\ \ (\sss : \DepHom_{\Alg_s}\ P) \\
  &\ \ \times (\sss_0 : (x : R_i (\extendfunc{U}^0_s X)) (y : F^1\ X\ x)\\
  &\ \ \ \ \ \ \to \liftdephom{U}^1_s\ \sss\ x\ (\theta\ x\ y) = m\ (x , \liftdephom{R_i \circ \extendfunc{U}^0_s}\ \sss\ x)\ (y , \liftdephom{F}^1\ \sss\ x\ y))
\end{align*}
%
...
$\ell, r : \Nat{F}{U}$ the end points of the path constructor. Let
...
$\ell_P : (x : FX) \times \Box_F\ P\ x \to (y : UX) \times \Box_U\ P\ y$
...
\begin{align*}
  ...
  &&&(\rho_0 : \pi_0 \circ \ell_P = r_P) \times (\rho_1 : \pathover{\lambda f . (x : FX) \times \Box_F P x \to \Box_U P (f x)}{\rho_0}{\pi_1 \circ \ell_P}{\pi_1 \circ r_P}) \\
  &=&&\reasontext{by definition and naturality of $\ell_P$ and $r_P$} \\
  &&&(\rho_0 : \ell_X = r_X) \times (\rho_1 : \pathover{\lambda f . (x : FX) \times \Box_F P x \to \Box_U P (f x)}{\rho_0}{\pi_1 \circ \ell_P}{\pi_1 \circ r_P}) \\
  &=&&\reasontext{by uniqueness of identity proofs, we can multiply by $(\rho_0 = \theta)$} \\
  &&&(\rho_0 : \ell_X = r_X) \times (\rho_0 = \theta) \times (\rho_1 : \pathover{\lambda f . (x : FX) \times \Box_F P x \to \Box_U P (f x)}{\rho_0}{\pi_1 \circ \ell_P}{\pi_1 \circ r_P}) \\
  &=&&\reasontext{singleton contraction} \\
  &&&(\pathover{\lambda f . (x : FX) \times \Box_F P x \to \Box_U P (f x)}{\rho_0}{\pi_1 \circ \ell_P}{\pi_1 \circ r_P}) \\
  &=&&\reasontext{function extensionality} \\
  &&&(x : FX) \times (y : \Box_F\ P\ x) \to \pathover{\Box_U P}{\theta x}{l^1_P\ (x , y)}{r^1_P\ (x , y)}
\end{align*}
%
...
\item $S_i : \Cat$ with $p : \sortsin{S_i}{\Ss}$
\item $F : \Func{\Alg_s}{S_i}$ such that $t_i \circ F = t_i \circ \extendfunc{U}_s$
\item $l , r : \Nat{F}{\extendfunc{U}_s}$ such that $t_i l = t_i r = \id$
\end{itemize}
...
$$
\theta :  (x : R_i (\extendfunc{U}^0_s X)) \to (y : F^1\ X\ x) \to \ell^1_X\ x\ y = r^1_X\ x\ y
$$
...
&(m : (x : R_i (\extendfunc{U}^0_s X)) \times (x' : \Box_{R_i \circ \extendfunc{U}^0_s}\ P\ x) \\
& \to (y : F^1\ X\ x) \times (y' : \Box_{F^1}\ P\ (x, x')\ y) \\
& \to \pathover{\Box_{\extendfunc{U}_s^1 P (x,x')}}{\theta\ x\ y}{\ell^1_P\ (x,x')\ (y,y')}{r^1_P\ (x,x')\ (y,y')})
\end{align*}
%
